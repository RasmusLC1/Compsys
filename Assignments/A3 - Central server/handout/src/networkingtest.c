...
void get_file(char* username, hashdata_t hash, char* to_get){
    ...
    if (statusCode != 1){
        FILE *filErrors;
        filErrors = fopen("zerrors.txt", "a");
        fprintf(filErrors, "fail\n");
        fclose(filErrors);
        return;
    }
    ...
    FILE *file;
    file = fopen(to_get, "a");
    int previousBlock = blockNumber+1;
    int n = 0;
    ...
    free(buffer);
    FILE *fileSuccess;
    fileSuccess = fopen("zsuccess.txt", "a");
    fprintf(fileSuccess, "success\n");
    fclose(fileSuccess);
}
...
int main(int argc, char **argv){
    ...
    // // test consistent names should succeed
    // char *usernamehold = "test";
    // char *passwordhold = "test";
    // memcpy(username, usernamehold, strlen(usernamehold));
    // memcpy(password, passwordhold, strlen(passwordhold));



    // //Test to see if we can get in with wrong password, password has already
    // //been set to test should fail
    // char *usernamehold = "test";
    // char *passwordhold = "test123";
    // memcpy(username, usernamehold, 16);
    // memcpy(password, passwordhold, 16);


    // //Test to see if we can set multiple users with same
    ...
    // //test random names using srand seeded with the current time should succeed
    struct timeval time; 
    gettimeofday(&time, NULL);
    srand((time.tv_sec) + (time.tv_usec));
    for (int i=0; i<USERNAME_LEN; i++) {  
    ...
    // //test random names using srand seeded with the current time that are
    //to long, should return error
    // struct timeval time; 
    // gettimeofday(&time, NULL);
    // srand((time.tv_sec) + (time.tv_usec));
    // for (int i=0; i<USERNAME_LEN+20; i++) {  
    //     username[i] = 'a' + rand() %26;
    // }
    // // Clean up password string as otherwise some extra chars can sneak in.
    // for (int  i=0; i<PASSWORD_LEN+20; i++) {
    //     password[i] = 'a' + rand() %26;
    // }


    // //test random password using srand seeded with the current time
    // //to see if we randomly get into the user, should return error
    // struct timeval time; 
    // gettimeofday(&time, NULL);
    // srand((time.tv_sec) + (time.tv_usec));
    // char *usernamehold = "test";
    ...
    // }
     ...
     // //Test to see what happens if you give empty username, should fail
    // char *usernamehold = "";
    // char *passwordhold = "test123";
    // memcpy(username, usernamehold, 16);
    // memcpy(password, passwordhold, 16);
     ...
     // //Test to see what happens if you give empty password should 
     // //work as salt becomes password, better practice would be
     // //to implement password length condition
    // char *usernamehold = "test1234";
    // char *passwordhold = "0";
    // memcpy(username, usernamehold, 16);
    // memcpy(password, passwordhold, 16);


    // //test random names using srand seeded with the current time but with 
    // //illegal characters, should return fail
    // struct timeval time; 
    // gettimeofday(&time, NULL);
    // srand((time.tv_sec) + (time.tv_usec));
    // for (int i=0; i<USERNAME_LEN; i++) {  
    //     username[i] = 'a' + rand() %100;
    // }
    // // Clean up password string as otherwise some extra chars can sneak in.
    // for (int  i=0; i<PASSWORD_LEN; i++) {
    //     password[i] = 'a' + rand() %100;
    // }
    ...
    srand(strlen(username)^6+4+strlen(password)*13);
