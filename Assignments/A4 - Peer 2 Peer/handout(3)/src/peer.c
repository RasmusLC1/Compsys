...
void send_message(PeerAddress_t peer_address, int command, char* request_body) {
    ...
    for (uint32_t b=0; b<ref_count; b++){
        ...
        if (reply_status != STATUS_OK){
            ...
            reply_status == STATUS_PEER_EXISTS){
    ...
    if (reply_status == STATUS_OK){
        if (command == COMMAND_REGISTER){
            ...
            //Setting up our variables used to get the network addresses
            int networkSize = reply_length/20;
            int peerPortI;
            char peerIP[IP_LEN];
            char peerPortC[10];
            printf("networksize: %d\n", networkSize);
            peer_count = networkSize;


            //Extract and convert the ip and port values from the replybody
            for (int i = 0; i < networkSize; i++){
                memcpy(peerIP, &reply_body[i*20], IP_LEN);
                peerPortI = ntohl(*(uint32_t*)&reply_body[i*20+16]);
                sprintf(peerPortC, "%d", peerPortI);

                //Store the IP and Port in the network array
                network[i] = malloc(sizeof(PeerAddress_t*));
                memcpy(network[i]->ip, peerIP, IP_LEN);
                memcpy(network[i]->port, peerPortC, PORT_LEN);                      
            }
...
void return_fail(int connfd, int status){
    struct ReplyHeader peerInfo;
    if (status != COMMAND_REGISTER){
        char buffer[REPLY_HEADER_LEN];
        peerInfo.length = htonl(0);
        peerInfo.status = htonl(status);
        peerInfo.this_block = htonl(0);
        peerInfo.block_count = htonl(1);
        hashdata_t placeholder;
        get_data_sha("payload", placeholder, peer_count*20, SHA256_HASH_SIZE);
        memcpy(peerInfo.block_hash, placeholder, SHA256_HASH_SIZE);
        memcpy(peerInfo.total_hash, placeholder, SHA256_HASH_SIZE);
        memcpy(buffer, &peerInfo, REPLY_HEADER_LEN);
        rio_writen(connfd, buffer, REPLY_HEADER_LEN);
        return;
    } 
    //Insert into buffer and send
    char buffer[MAX_MSG_LEN];

    //Setting up the header
    struct NetworkAddress replyBody[peer_count-1];
    peerInfo.length = htonl(peer_count*20-20);
    peerInfo.status = htonl(status);
    peerInfo.this_block = htonl(0);
    peerInfo.block_count = htonl(1);


    //Adding the network to replybody and into a payload buffer
    uint32_t PortConverter;
    for (uint32_t i = 0; i < peer_count; i++){
        memcpy(replyBody[i].ip, &network[i]->ip, IP_LEN);
        PortConverter = atoi(network[i]->port);
        replyBody[i].port = htonl(PortConverter);
    }
    //Setting up our payload
    int ipIndex;
    char payload[peer_count*20+1];
    for (uint32_t i = 0; i < peer_count; i++){
        ipIndex = i*20;
        memcpy(payload + ipIndex, &replyBody[i], IP_LEN + PORT_LEN);
    }
    // Hashing
    hashdata_t placeholder;
    get_data_sha(payload, placeholder, peer_count*20-20, SHA256_HASH_SIZE);
    memcpy(peerInfo.block_hash, placeholder, SHA256_HASH_SIZE);
    memcpy(peerInfo.total_hash, placeholder, SHA256_HASH_SIZE);

    //Loading into buffer
    memcpy(buffer, &peerInfo, REPLY_HEADER_LEN);
    memcpy(buffer + REPLY_HEADER_LEN, &payload, sizeof(payload));

    
    //Writing to the client, closing connection and releasing lock
    rio_writen(connfd, buffer, REPLY_HEADER_LEN + peer_count*20);
    Close(connfd);
...
void handle_register(int connfd, char* client_ip, int client_port_int){
    ...
    char clientString[5];
    sprintf(clientString, "%d", client_port_int);


    //Error handling
    if (sizeof(client_ip)*2 != IP_LEN){
        printf("Not real IP address: %ld, %s", sizeof(client_ip), client_ip);
        return_fail(connfd, STATUS_MALFORMED);
        pthread_mutex_unlock(&network_mutex);
        return;
    } else if (sizeof(client_port_int)*2 != PORT_LEN){
        printf("Not real Port");
        return_fail(connfd, STATUS_MALFORMED);
        pthread_mutex_unlock(&network_mutex);
        return;
    }

    //Checking if the peer is already in the network
    for (uint32_t i = 0; i < peer_count; i++){
        if(memcmp(network[i]->ip, client_ip, 9) == 0 &&
        memcmp(network[i]->port, clientString, 5) == 0){
            printf("PEER EXISTS\n");
            return_fail(connfd, COMMAND_REGISTER);
            pthread_mutex_unlock(&network_mutex);
            return;
        }
    }
    ...
    //Set up information to send, we allocate memory for each element, should
    //probably be realloc but we can't get it to work
    network[peer_count] = malloc(sizeof(PeerAddress_t*));
    memcpy(network[peer_count]->ip, client_ip, IP_LEN);
    memcpy(network[peer_count]->port, clientString, PORT_LEN);
    

    //Setting up the header
    char buffer[MAX_MSG_LEN];
    struct NetworkAddress replyBody[peer_count];
    struct ReplyHeader peerInfo;
    peer_count ++;
    peerInfo.length = htonl(peer_count*20);
    peerInfo.status = htonl(status);
    peerInfo.this_block = htonl(0);
    peerInfo.block_count = htonl(1);
    ...
    //Setting up buffer with new IP
    char peerBuffer[IP_LEN+PORT_LEN+1];
    char clientPortStr[PORT_LEN];
    memcpy(peerBuffer, client_ip, IP_LEN);
    sprintf(clientPortStr, "%d", client_port_int);
    memcpy(peerBuffer+strlen(client_ip), clientPortStr, PORT_LEN);    

    PeerAddress_t existingPeer;
    for (uint32_t i = 0; i < peer_count-1; i++){
        memcpy(existingPeer.ip, network[i]->ip, IP_LEN);
        memcpy(existingPeer.port, network[i]->port, PORT_LEN);
        send_message(existingPeer, COMMAND_INFORM, peerBuffer);
    }
    return;
}
...
void handle_inform(char* request){
    ...
    char *clientPort[PORT_LEN];
    memcpy(clientIP, request, 9);
    memcpy(clientPort, &request[9], 5);

    //Comparator to see if the new peer is already in network;
    for (uint32_t i = 0; i < peer_count; i++){
        if(strcmp(network[i]->ip, clientIP) &&
        strcmp(network[i]->port, clientPort) ){
            printf("Already in network\n");
            return;
        }
    }
    
    //Insert the new IP at the end of our current networklist
    network[peer_count] = malloc(sizeof(PeerAddress_t));
    memcpy(network[peer_count] -> ip, clientIP, IP_LEN);
    memcpy(network[peer_count] -> port, clientPort, PORT_LEN);
    peer_count++;
    return;
...
/*
 * Handle 'retreive' type messages as defined by the assignment text. This will
...
void handle_retreive(int connfd, char* request){
    ...
    //Opening file and checking for existence
    FILE *file;
    file = fopen(request,"r");
    if (file == NULL) {
        printf("File %s does not exist, handle_retreive", request);
        return_fail(connfd, STATUS_MALFORMED);
        pthread_mutex_unlock(&retrieving_mutex);
        return;
    }

    //Setting up our variables to 0 for safety
    ...
    totalChar = ftell(file); 
    totalBlocks = totalChar/8116;
    printf("total blocks: %d\n", totalChar);

    //Assemble the fixed parts of the package;
    hashdata_t blockHash, file_hash;
    ReplyHeader_t package;
    package.status = htonl(status);
    package.block_count = htonl(totalBlocks);
    
    char payload[REPLY_HEADER_LEN];
    char data[MAX_MSG_LEN];
    //fseek relevant section of file and extract the chars and assemble rest of
    //package
        
    //Get the data from the file, fseek to return to start
    fseek(file, currentChar, 0);
    printf("Block number: %d\n", currentBlock);
    for (uint32_t i = 0; i < totalChar; i++){
        data[length++] = fgetc(file);

        //Condition to check if we have reached the Max msg limit
        if(length == 8116){
            //Setting up the package
            currentBlock++;
            package.length = htonl(length);
            package.this_block = htonl(currentBlock);
            get_file_sha(request, file_hash, SHA256_HASH_SIZE);
            get_data_sha(data, blockHash, length, SHA256_HASH_SIZE);
            memcpy(package.block_hash, blockHash, SHA256_HASH_SIZE);
            memcpy(package.total_hash, file_hash, SHA256_HASH_SIZE);
            memcpy(payload, &package, REPLY_HEADER_LEN);
            
            //Writing to the client twice as the data is sent
            //seperate from the payload
            rio_writen(connfd, payload, REPLY_HEADER_LEN);
            rio_writen(connfd, data, length);
            length = 0;
        }

    }
    //Setting up the final package outside loop
    currentBlock++;
    printf("legnth %d\n\n", length);
    package.length = htonl(length);
    package.block_count = htonl(currentBlock);
    get_file_sha(request, file_hash, SHA256_HASH_SIZE);
    get_data_sha(data, blockHash, length, SHA256_HASH_SIZE);
    memcpy(package.block_hash, blockHash, SHA256_HASH_SIZE);
    memcpy(package.total_hash, file_hash, SHA256_HASH_SIZE);
 ...
 */
void handle_server_request(int *connection){
    ...
    char requestHeader[REQUEST_HEADER_LEN];
    memcpy(requestHeader, buffer, REQUEST_HEADER_LEN);

    //Getting header from message
    char clientIP[IP_LEN];
    memcpy(clientIP, buffer, IP_LEN);
    uint32_t clientPort = ntohl(*(uint32_t *)&requestHeader[16]);
    uint32_t commandCode = ntohl(*(uint32_t *)&requestHeader[20]);
    uint32_t length = ntohl(*(uint32_t *)&requestHeader[24]);

    //second connection to get the body
    ...
    if (strlen(request) != length) {
        printf("STATUS_MALFORMED length, handle_server_request %d %ld\n, requests: %s\n", length, sizeof(request), request);
        return;
    } else if (commandCode != COMMAND_REGISTER && commandCode != COMMAND_INFORM
    && commandCode != COMMAND_RETREIVE){
        printf("STATUS_MALFORMED command, handle_server_reques %d", commandCode);
        return;
    }
    
    
    //Send to correct function depending on commandcode
    switch (commandCode){
        case COMMAND_REGISTER:
            handle_register(connfd, clientIP, clientPort);
            break;
        case COMMAND_INFORM:
            printf("\n\nrequest\n\n");
        ...
        case COMMAND_RETREIVE:
            ...
            break;
        default:
            printf("Something went wrong, handle_server_request");
            break;
    }
    return;
}
...
void* server_thread(){
    ...
    while (1) {
	    ...
	    *connfd = Accept(listenfd, (SA *) &clientaddr, &clientlen); 
	    Pthread_create(&tid, NULL,handle_server_request, connfd);
    }
    exit(0);
