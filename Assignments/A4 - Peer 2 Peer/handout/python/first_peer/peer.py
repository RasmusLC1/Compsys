...
class RequestHandler(socketserver.StreamRequestHandler):
    ...
    # Looks like handle_server_request
    def handle(self):
        """
    ...
    def _register(self, ip:str, port: int):
        ...
        print(f"Network is: {', '.join(network)}")
    ...
    def _build_and_send_responses(self, status:int, to_send: bytes):
        ...
        for block_num, block_payload in blocks:
            ...
            # An artificial delay, to make it easier to test that server/client 
            # interactions are happening at the same time, as well as locking 
            # of shared resources. This is not included by default but you may
            # wish to add it to ensure that a system is performing correctly in
            # a busy network, without having to actually start 100,000 peers at
            # once or sending 1PB files.
#            time.sleep(5)
    ...
    if status == STATUS_OK:
        if code == COMMAND_REGISTER:
            ...
            try:
                ...
                print(f"Got network: {', '.join(network)}")
                network_mutex.release()
...
if __name__ == "__main__":
    ...
    network.append(f"{configs['my_ip']}:{configs['my_port']}")
